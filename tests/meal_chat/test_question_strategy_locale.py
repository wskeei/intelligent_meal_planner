...
def test_build_follow_up_plan_uses_readable_english_goal_text_for_low_confidence():
    memory = ConversationMemory(
        known_facts={"locale": "en"},
        preferences={"health_goal": "lose_weight", "budget": 100},
    )
    ...
    assert "lose_weight" not in follow_up.assistant_message
    assert '"fat loss"' in follow_up.assistant_message
