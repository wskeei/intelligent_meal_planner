# 基于 DQN 与多 Agent 协作的智能配餐系统

## 项目简介

本项目是一个智能配餐系统，当前运行时主流程已经切换为“单一营养师人格前台 + 后台 CrewAI 多 agent 协作 + DQN 配餐”的模式。
...
1. 通过自然对话理解目标、预算、忌口和口味偏好
2. 后端 Crew runtime 维护结构化会话记忆，并把资料映射成营养范围，而不是单点值
3. 当预算偏紧时进入可恢复的协商阶段，默认给出两种折中方向
4. 条件满足后调用 DQN 规划器，生成正式方案或双候选方案
