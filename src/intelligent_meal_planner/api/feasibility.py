...
class FeasibilityService:
    ...
    def __init__(self):
        ...
        # 按餐次分类
        self.breakfast = [
            r for r in self.recipes if "breakfast" in r.get("meal_type", [])
        ]
    ...
    def _compute_max_achievable(
        self, budget: float, items_per_meal: int = 2
    ) -> Dict[str, int]:
        """
        贪心算法计算给定预算下的最大可达营养值
        ...
        # 按热量/价格比排序每个餐次的菜品
        bf_sorted = sorted(
            self.breakfast, key=lambda x: x["calories"] / x["price"], reverse=True
        )
        lu_sorted = sorted(
            self.lunch, key=lambda x: x["calories"] / x["price"], reverse=True
        )
        dn_sorted = sorted(
            self.dinner, key=lambda x: x["calories"] / x["price"], reverse=True
        )
    ...
    def check_feasibility(
        ...
        # 计算各指标可达性(目标能达成的百分比)
        cal_feas = (
            min(100.0, (max_vals["calories"] / target_calories * 100))
            if target_calories > 0
            else 100
        )
        pro_feas = (
            min(100.0, (max_vals["protein"] / target_protein * 100))
            if target_protein > 0
            else 100
        )
        carb_feas = (
            min(100.0, (max_vals["carbs"] / target_carbs * 100))
            if target_carbs > 0
            else 100
        )
        fat_feas = (
            min(100.0, (max_vals["fat"] / target_fat * 100)) if target_fat > 0 else 100
        )
