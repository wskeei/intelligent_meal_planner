<template>
  <div class="profile-page">
    ...
    <section v-if="missingProfileFields.length" class="missing-bar">
      <button class="missing-bar__toggle" type="button" @click="showMissing = !showMissing">
    ...
    <section class="form-section">
      ...
      <div class="field-grid">
        <div class="field">
          ...
          <el-input-number v-model="localProfile.age" :min="10" :max="100" controls-position="right" />
          <p class="field__hint">{{ $t('profile.field_reasons.age') }}</p>
        </div>
        <div class="field">
          <label class="field__label">{{ $t('auth.activity_level') }}</label>
          <el-select v-model="localProfile.activityLevel" :placeholder="$t('auth.activity_level')">
      ...
      <div class="field-grid">
        <div class="field">
          ...
          <el-input-number v-model="localProfile.height" :min="100" :max="250" controls-position="right" />
          <p class="field__hint">{{ $t('profile.field_reasons.height') }}</p>
        </div>
        <div class="field">
          <label class="field__label">{{ $t('auth.weight') }}</label>
          <el-input-number v-model="localProfile.weight" :min="30" :max="250" :step="0.5" controls-position="right" />
